...
user = "${session_user}"
EOF
  # The user's starter config is Hyprland's own example/hyprland.lua at
  # the resolved tag — the full upstream keybind set, rules, and docs —
  # not a hand-rolled minimal file that drifts from upstream. The staged
  # source tree is guaranteed present: phase_hyprland stages hyprland
  # before calling this, and the build-dep purge that deletes the tree
  # runs after. No autogenerated marker: the appended welcome app IS the
  # first-run notice.
  ...
  cp "${example}" "${TARGET}/home/${TARGET_USERNAME}/.config/hypr/hyprland.lua"
  cat >>"${TARGET}/home/${TARGET_USERNAME}/.config/hypr/hyprland.lua" <<'EOF'

-- --- Hypr-Deb additions (installer.sh) ---------------------------------------
-- First-run welcome, replacing upstream's autogenerated-config notice.
-- The marker lands only when the app exits cleanly (the user closed it),
-- so the welcome keeps appearing until actually acknowledged. Lua long
-- brackets keep the sh quoting sane.
