...
**Architecture:** `installer.sh` is a thin orchestrator that sources `lib/`
...
- Modules in `lib/` and `scripts/` start with `# shellcheck shell=bash`, no
  ...
  `installer.sh`, `tools/check.sh`, and `tests/*.sh` have
...
# Static config written by installer.sh; regenerated by hypr-deb-sync-esp.
...
### Task 14: `installer.sh` — orchestrator
...
out="$(bash installer.sh --yes --bootloader=grub 2>&1 || true)"
...
```bash
git add installer.sh tests/orchestrator.sh
...
- Usage: `sudo ./installer.sh` (prompts for bootloader and destruction),
