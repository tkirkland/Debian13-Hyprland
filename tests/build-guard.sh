...
# shellcheck source=tools/lib-build-guard.sh
...
# shellcheck disable=SC2317  # called indirectly by assert_chrooted_in_target
in_target(){ /usr/bin/env bash -c "$1"; }      # no-chroot fallback
assert_fails "no-chroot in_target rejected" assert_chrooted_in_target
# shellcheck disable=SC2153  # TARGET is the installer's cross-module global, set at runtime
